...
import type { AuthenticationService } from '../../application/services/AuthenticationService.js';
...
function parseExpiryToMs(expiresIn: string): number {
  ...
  return value * MS_MULTIPLIERS[match[2]];
}
...
export class AuthController {
  ...
  refreshAccessToken = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (message.includes('Invalid') || message.includes('logged out')) {
        logger.warn('Refresh attempt with invalid/revoked token', { error: message });
  ...
  validateApiKey = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!apiKey || !apiSecret) {
        res.status(400).json({ error: 'Missing API key or secret' });
        return;
      }
