...
const logFormat = winston.format.printf((info) => {
    ...
    return `[${timestamp}] [${String(level).toUpperCase()}] [${label}]: ${message}${metaString}`;
});
const logger = winston.createLogger({
    level: 'info', // info, warn, error
    format: winston.format.combine(winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), winston.format.label({ label: 'Core Service' })),
    transports: [
