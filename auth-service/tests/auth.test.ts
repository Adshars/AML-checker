...
describe('Auth Service Integration Tests', () => {
    ...
    describe('POST /auth/refresh', () => {
        ...
        it('should refresh token (Success) -> 200 + new AccessToken + rotated cookie', async () => {
            const validRefreshToken = jwt.sign({ userId: 'u1' }, process.env.REFRESH_TOKEN_SECRET as string, { expiresIn: '1h' });
        ...
        it('should fail (Token not in DB / Logged Out) -> 403', async () => {
            const validRefreshToken = jwt.sign({ userId: 'u1' }, process.env.REFRESH_TOKEN_SECRET as string);
